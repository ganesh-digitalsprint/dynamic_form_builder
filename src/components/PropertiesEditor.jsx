...
const PropertiesEditor = ({
  ...
  // Load options from localStorage when component mounts or field changes
  useEffect(() => {
    const savedData = getOptionsFromStorage(
      selectedField.key || selectedField.label
    );
    if (savedData.length) {
      onUpdateField(selectedField.id, { options: savedData });
    }
  }, [selectedField.id]);
  ...
  const saveOptionsToLocal = (newOptions) => {
    onUpdateField(selectedField.id, { options: newOptions });
    saveOptionsToStorage(selectedField.key || selectedField.label, newOptions);
  };

  const handleOptionsChange = (index, value) => {
    const newOptions = [...selectedField.options];
    newOptions[index] = value;
    saveOptionsToLocal(newOptions);
  };
      ...
      <div className="space-y-4">
        ...
        {selectedField.type === "textarea" && (
          <div>
            ...
            <input
              ...
              onChange={(e) =>
                handlePropertyChange("rows", parseInt(e.target.value))
              }
              className="w-full px-3 py-2 border rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent"
            />
          </div>
        ...
        <div>
          ...
          <select
            ...
            onChange={(e) =>
              handlePropertyChange("gridSpan", parseInt(e.target.value))
            }
            className="w-full px-3 py-2 border rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent"
          >
